...
export const existingUser = async (email, next, password) => {
...
export const validationRegistrationData = async (data, userType) => { };
// validation user data
export const checkOtpRestrictions = async (email, next) => {
    if (await redis.get(`otp_lock:${email}`)) {
        return next(new ErrorHandler("Account locked due to multiple failed attempts ! Try again after 30 mintues", 500));
    }
    if (await redis.get(`otp_spam_lock:${email}`)) {
        return next(new ErrorHandler("Too may OTP requests! Please wait 1hours before requesting again", 500));
    }
    if (await redis.get(`otp_cooldown:${email}`)) {
        return next(new ErrorHandler("Please wait 1minute before requesting a new otp!", 500));
    }
};
// send Otp
export const sendOtp = async (email, template, name) => {
...
export const verifyOtp = async (email, otp, next) => {
    ...
    if (isLocked) {
        return next(new ErrorHandler("Account locked due to multiple failed OTP attempts! Try again after 30 minutes.", 400));
    }
...
export const verifyForgetPassword = async (req, res, next, userType) => {
    try {
