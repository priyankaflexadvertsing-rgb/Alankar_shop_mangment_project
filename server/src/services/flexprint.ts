...
export const flexPrint = (items: any, rate: any) => {
  ...
  const regex =
    /^(\d+(?:\.\d+)?)(feet|inch)?x(\d+(?:\.\d+)?)(feet|inch)?=(\d+)(normal|star|vinyl|blackback)\.jpg$/i;
  ...
  if (!widthUnit && heightUnit) widthUnit = heightUnit;
  if (!heightUnit && widthUnit) heightUnit = widthUnit;

  // Case 3: mixed units → convert inches to feet
  if (widthUnit === "inch") width *= INCH_TO_FEET;
  if (heightUnit === "inch") height *= INCH_TO_FEET;
