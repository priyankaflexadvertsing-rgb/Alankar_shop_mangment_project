...
inputPath: string, outputPath: string): Promise<string | null> => {
  try {
    ...
    try {
      ...
      if (metadata?.DateTimeOriginal) {
        await exiftool.write(outputPath, {
          AllDates: metadata?.DateTimeOriginal as any,
        });
      }
    } catch (exifErr) {
      const err = exifErr as Error;
      console.warn(`⚠️ Failed to copy EXIF date: ${err.message}`);
    }
...
class ReadDirRecursive {
  static readDirectoryRecursive(dir: string): string[] {
    const result: string[] = [];
