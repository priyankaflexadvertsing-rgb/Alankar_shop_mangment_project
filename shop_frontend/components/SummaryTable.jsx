...
export const SummaryTable = ({ printing, userId, role }) => {
    ...
    // DELETE API
    const deleteUserFile = async ({ printingId, userId }) => {
                            ...
                            <>
                                {printing.map((it, idx) => (
                                    <tr key={idx} className="bg-gray-800 text-white hover:bg-gray-700">
                                        ...
                                        <td className="p-2 border flex items-center"><img
                                            src={`${SOCKET_URI}/thumbnails/${encodeURIComponent(it.compressedfilePath.split("\\").pop())}`}
                                            alt={`Thumbnail ${idx + 1}`}
                                        ...
                                        {/* DELETE BUTTON */}
                                         <td
                                            ...
                                            onClick={() => deleteUserFile({ printingId: it._id, userId: userId })}
                                        >
                                            Delete
                                        </td>
